<template>
  <div v-if="showSuggestions && suggestions.length > 0" class="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-xl z-50 mt-2 overflow-hidden">
      ...
      >
        ...
        <div class="flex-shrink-0">
          <img 
            ...
            @error="(event) => image.handleImageError(event, suggestion.media_type === 'person' ? 'profile' : 'poster')"
          />
          <div v-else class="w-12 h-16 bg-gray-200 rounded-lg flex items-center justify-center shadow-sm">
            <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
              <path v-if="suggestion.media_type === 'person'" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" />
              <path v-else d="M2 6a2 2 0 012-2h6l2 2h6a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" />
            </svg>
          </div>
        </div>
        ...
        <div class="flex-1 min-w-0">
          ...
          <p v-if="suggestion.overview" class="text-sm text-gray-600 mb-2 line-clamp-2">
            {{ suggestion.overview }}
          </p>
          
          <div class="flex items-center space-x-3 text-xs text-gray-500">
            <span v-if="suggestion.release_date || suggestion.first_air_date" class="flex items-center">
            ...
            <span v-if="suggestion.known_for_department" class="flex items-center">
              <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
