...
const props = defineProps({
  ...
  inputClass: {
    type: String,
    default: 'w-full px-4 py-3 pl-10 pr-20 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent'
  },
  // 是否显示搜索按钮
  showSearchButton: {
    type: Boolean,
...
const debouncedSearch = common.debounce(async (query) => {
  try {
    ...
    if (result.data.value) {
      searchSuggestions.value = result.data.value.results.slice(0, props.suggestionLimit)
