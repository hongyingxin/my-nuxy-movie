            ...
            <div class="flex items-center space-x-2">
              ...
              <div class="flex space-x-1">
                <button
                  ...
                  :class="selectedMediaTypes.includes(type.value)
          ...
          <div class="flex justify-between items-center mb-6">
            <div class="flex items-center space-x-2">
              <button
                ...
                :class="viewMode === 'grid' ? 'bg-red-100 text-red-600' : 'text-gray-400 hover:text-gray-600'"
              >
              ...
              <button
                @click="viewMode = 'list'"
                class="p-2 rounded-md transition-colors"
                :class="viewMode === 'list' ? 'bg-red-100 text-red-600' : 'text-gray-400 hover:text-gray-600'"
              >
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
...
// ==================== API 导入 ====================
import { multiSearch, advancedMultiSearch } from '~/api/search'
...
const mediaResults = computed(() => {
  return searchResults.value.filter(item => item.media_type === 'movie' || item.media_type === 'tv')
})
...
const performSearch = async () => {
  ...
  try {
    ...
    if (Array.isArray(result)) {
      ...
      // 按相关性排序
      searchResults.value = allResults.sort((a, b) => b.popularity - a.popularity)
      totalResults.value = total
      totalPages.value = Math.max(...result.map(r => r.data.value?.total_pages || 0))
    } else {
      // 单个搜索结果（multi search）
...
onMounted(() => {
  ...
  if (q) {
    ...
    selectedMediaTypes.value = types ? types.split(',') : ['movie', 'tv', 'person']
...
watch(() => route.query, (newQuery) => {
  if (newQuery.q && hasSearched.value) {
    ...
    selectedMediaTypes.value = newQuery.types ? newQuery.types.split(',') : ['movie', 'tv', 'person']
