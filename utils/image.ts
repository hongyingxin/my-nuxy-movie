...
function getTmdbImageUrl(
  ...
  if (!path) {
    ...
    return (
      PLACEHOLDER_SVGS[type]?.[
        fallbackSize as keyof (typeof PLACEHOLDER_SVGS)[typeof type]
      ] || PLACEHOLDER_SVGS.poster.medium
    )
  }
  ...
  const sizeValue =
    sizeConfig[actualSize as keyof typeof sizeConfig] || sizeConfig.medium
 ...
 */
function getResponsiveImageUrls(
  path: string | null,
  type: ImageType = 'poster'
): Array<{ size: string; url: string }> {
  if (!path) return []
